...
class BookService:
    @staticmethod
    async def import_book_from_open_library(db: Session, book: BookImport) -> Book | None:
        isbn_exists = BookService._get_book_from_db(db, book.isbn)
        if isbn_exists:
            raise HTTPException(status_code=409, detail=f"Book ISBN: {book.isbn} already exists in the database.")
    ...
    def add_book_stock(isbn: str, stock: int, db: Session) -> Book | None:
        book = BookService._fetch_book_and_validate(db, isbn)
        book.stock += stock
        db.commit()
        db.refresh(book)
        return book
    ...
    def update_book(updated_book: BookPut, db: Session, target_isbn: str) -> Book:
        book_db = BookService._fetch_book_and_validate(db, target_isbn)
    ...
    @staticmethod
    def _fetch_book_and_validate(db: Session, isbn: str) -> Book:
